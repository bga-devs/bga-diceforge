...
class Game extends \Bga\GameFramework\Table implements ResourceChoiceDb
{
    ...
    public const MAX_MOONSHARD = 6;
    public const MAX_ANCIENTSHARD = 6;
    public const CHEST_GOLD = 4;
    public const CHEST_FIRESHARD = 3;
    public const CHEST_MOONSHARD = 3;
    public const HAMMER_MAX_POSITION = 30;
    ...
    public array $exploit_cost;
    public array $decks;
    public array $ressourceToText;
    public array $shields;
    public array $exploit_types;
    public array $dice_sides;
    public array $celestialDie ;
    public array $celestialInfo;
    public array $sides_init;
    public array $pools;
    public array $powers;
    public array $powers_you;
    public array $maze;
    public array $titan_passives;
    public array $titan_board;
    ...
    public function __construct()
    {
        ...
        // Card init
        $this->exploits = $this->deckFactory->createDeck('exploit');
    ...
    */
    protected function setupNewGame($players, $options = [])
    {
        ...
        $sql =
            'INSERT INTO player (player_id, player_color, player_canal, player_name, player_avatar, player_score) VALUES ';
        $values = [];
        foreach ($players as $player_id => $player) {
            ...
            $tokensToInit[] = [
                'key' => 'triple_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'throw1_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'throw2_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'mirror1_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'mirror2_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
            ...
            $tokensToInit[] = [
                'key' => 'moonshard_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'fireshard_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'ancientshard_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'loyalty_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'cerberus_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'resolveTwin_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'twinChoice_' . $player_id,
                'nbr' => 1,
                'state' => 0,
            ];
            ...
            if ($rebellion == 4) {
                $tokensToInit[] = [
                    'key' => 'position_' . $player_id,
                    'nbr' => 1,
                    'state' => 1,
                ];
                $tokensToInit[] = [
                    'key' => 'mazestock_' . $player_id,
                    'nbr' => 1,
                    'state' => 0,
                ];
                $tokensToInit[] = [
                    'key' => 'mazechoice_' . $player_id,
                    'nbr' => 1,
                    'state' => 0,
                    'location' => '0',
                ];
                $tokensToInit[] = [
                    'key' => 'puzzle_' . $player_id,
                    'nbr' => 1,
                    'state' => 0,
                ];
            }
        ...
        if ($rebellion == 4) {
            $tokensToInit[] = [
                'key' => 'treasure_fireshard',
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
                'key' => 'treasure_moonshard',
                'nbr' => 1,
                'state' => 0,
            ];
            $tokensToInit[] = [
            ...
            foreach ($mandatory_deck as $island => $cards) {
                ...
                if (substr($cards[0]['type'], -10) == 'Misfortune') {
                    ...
                    foreach ($cards as $pos => $card) {
                        if (
                            substr($card['type'], -10) == 'Misfortune' &&
                            $test < $nb_players
                        ) {
                            $c[] = $card;
                            $test++;
        ...
        foreach ($this->loadPlayersBasicInfos() as $player_id => $players) {
            ...
            if ($rebellion == 3) {
                // dice 1
                $sides = [];
                $sides[] = ['type' => 'AS1', 'type_arg' => '', 'nbr' => '1'];
                $sides[] = ['type' => 'G1', 'type_arg' => '', 'nbr' => '5'];
                $this->sides->createCards($sides, 'dice1-p' . $player_id);

                // dice 2
                $sides = [];
                $sides[] = ['type' => 'MS1', 'type_arg' => '', 'nbr' => '1'];
                $sides[] = ['type' => 'L1V1', 'type_arg' => '', 'nbr' => '1'];
                $sides[] = ['type' => 'G1', 'type_arg' => '', 'nbr' => '4'];
    ...
    */
    protected function getAllDatas()
    {
        ...
        foreach (
            $this->exploits->countCardsInLocations() as $location => $number
        ) {
        ...
        // Sides information
        foreach (
            $this->sides->countCardsInLocations() as $location => $number
        ) {
        ...
        foreach ($result['players'] as $player_id => $player) {
            ...
            foreach (
                ...
                if (!isset($result['playersDice'][$player_id]['dice1'])) {
                    $result['playersDice'][$player_id]['dice1'] = [];
                }
            ...
            foreach (
                ...
                if (!isset($result['playersDice'][$player_id]['dice2'])) {
                    $result['playersDice'][$player_id]['dice2'] = [];
                }
            ...
            $result['players'][$player_id]['nbHammer'] = $this->hasActiveHammer(
                $player_id
            );
            $result['players'][$player_id]['remainingHammer'] = ceil(
                $this->remainingHammer($player_id) / self::HAMMER_MAX_POSITION
            );
            $player_chests = $this->hasChest($player_id);
            $result['players'][$player_id]['gold_max'] =
                self::MAX_GOLD + self::CHEST_GOLD * $player_chests;
            $result['players'][$player_id]['moon_max'] =
                self::MAX_MOONSHARD + self::CHEST_MOONSHARD * $player_chests;
            $result['players'][$player_id]['fire_max'] =
                self::MAX_FIRESHARD + self::CHEST_FIRESHARD * $player_chests;

            $result['powerTokens'][
                $player_id
            ] = $this->tokens->getTokensOfTypeInLocation(null, $player_id);
        }
        ...
        if ($result['rebellion'] == 3) {
            ...
            $result['memoryTokens'] = $this->tokens->getTokensOfTypeInLocation(
                '%Memory%_'
            );
        }

        if ($result['rebellion'] == 4) {
            $result['treasures'] = $this->tokens->getTokensOfTypeInLocation(
                'treasure_%'
            );
    ...
    */
    public function getPlayersRessources($player_id = null)
    {
        $counters = [];
        $rebellion = $this->getGameStateValue('rebellion');

        $sql =
            'SELECT player_id id, res_fire fire, res_moon moon, res_gold gold, res_ancient ancient, hammer_position%' .
            self::HAMMER_MAX_POSITION / 2 .
            ' as hammer_position, player_score FROM player';
        if ($player_id) {
        ...
        foreach ($query_arr as $player_id => $player) {
            ...
            $counters['discarded_sides_p' . $current_player] = [
                ...
                'counter_value' => count(
                    $this->sides->getCardsInLocation('discard', $player_id)
                ),
            ];
            ...
            $counters['card_counter_p' . $current_player] = [
                ...
                count(
                    ...
                    count(
                        $this->exploits->getCardsInLocation(
                            'pile2-' . $current_player
                        )
                    ) +
                    count(
                        $this->exploits->getCardsInLocation(
                            'pile3-' . $current_player
                        )
                    ),
            ];
        }
        ...
        foreach (
            $this->tokens->getTokensOfTypeInLocation('scepter') as $key => $token
        ) {
     ...
     */
    public function getPlayerReinforcements($player_id)
    {
        $data = [];
    ...
    */
    public function getGameProgression()
    {
        ...
        $currentProgression =
            ($turnCount - 1) * $progressionTurn +
            ($currentPlayerNum - 1) * $progressionPlayer;
    ...
    public function getSideIdFromType($side)
    {
    ...
    {
        $sql =
            "SELECT DISTINCT 1 FROM token WHERE token_state != 0 AND token_key LIKE 'mazestock_%'";
    ...
    public function generateActionChoiceArgs($player_id, $sides)
    {
        ...
        if ($this->getGameStateValue('celestialRunning')) {
            ...
            $players_info[$player_id]['side_choice_1'] != '0' ||
            $players_info[$player_id]['side_choice_2'] != '0'
        ) {
            //$i = 0;
            $sides[$player_id]['action'] = 'actionChoice';
            if (!$this->tokens->getTokenState("cerberus_$player_id")) {
    ...
    public function generateMazeChoiceArgs($player_id, $sides)
    {
        ...
        if (
            ...
            $sides[$player_id]['sides'][0] = [
                'id' => '',
                'type' => $this->tokens->getTokenLocation(
                    'mazechoice_' . $player_id
                ),
                'num' => 99,
            ];
            $sideNum = 99;
            ...
            !$this->getGameStateValue('mazeForcePath')
        ) {
            ...
            foreach ($way as $i) {
                ...
                if ($this->maze[$i]['reward'] == 'treasure') {
                    ...
                    $tok = $this->tokens->getTokensOfTypeInLocation(
                        'treasure%',
                        $i
                    );
    ...
    public function resetThrowTokens($player_id = null)
    {
        if ($player_id == null) {
    ...
    public function initNotif($player_id)
    {
        $notifPlayerArgs = [];
    ...
    public function possibilityAddText($possibilities)
    {
    ...
    public function getGoldResource($player_id, $type = 'all')
    {
        $gold = 0;
    ...
    public function getTridentSides()
    {
                    ...
                    foreach ($aux as $score_aux) {
                        ...
                        foreach ($players_being_tied as $player_id) {
                            switch ($act) {
                                case 'sides':
                                    $sql =
                                        'UPDATE player SET player_score_aux = 300 + ' .
                                        count(
                                            $this->sides->getCardsInLocation(
                                                'discard',
                                                $player_id
    ...
    */
    ...
    public function getExploitsMaterial($nb_players)
    {
        $setup_cards = [];
        ...
        foreach ($this->decks as $type => $cards) {
            foreach ($cards as $name) {
                ...
                if (
                    substr($name, -4) == 'Boar' ||
                    substr($name, -10) == 'Misfortune' ||
                    substr($name, -6) == 'Memory'
                ) {
                    if (
    ...
    public function getAvailableExploits($setup_cards)
    {
        $deckOption = $this->getGameStateValue('deckOption');
        $promoCards = $this->getGameStateValue('promoCards');
        $rebellion = $this->getGameStateValue('rebellion');
        ...
        elseif ($deckOption == 2 && $promoCards == 1) {
            $available_cards = array_merge_recursive(
                $setup_cards['random'],
                $setup_cards['promo']
            );
        }
        ...
        // French cup 2022 final
        elseif ($deckOption == 13) {
            $available_cards = $setup_cards['challenge2022final'];
        }

        // add rebellion cards to the random/draft (also with modules)
        if (
            $rebellion == 2 ||
            (($rebellion == 3 || $rebellion == 4) &&
                ($deckOption == 2 || $deckOption == 3))
        ) {
            $available_cards = array_merge_recursive(
                $available_cards,
                $setup_cards['rebellion_cards']
            );
        }
    ...
    public function draftSlot()
    {
        ...
        for ($i = 0; $i < count($this->exploit_slot); $i++) {
            ...
            if (
                $this->exploits->countCardInLocation($slot_to_draft[$slot]) == 0
            ) {
    ...
    public function rollDice($player_id, $dice_num)
    {
        ...
        if ($value != 0) {
            ...
            $new_side = $this->sides->getCardsInLocation(
                'dice' . $dice_num . '-p' . $player_id,
                $value
            );
    ...
    public function rollCelestial($player_id, $roll = true)
    {
        ...
        $notifPlayerArgs = $this->affectRessource(
            $player_id,
            $notifPlayerArgs,
            98,
            $side,
        ...
        if (
            isset($notifPlayerArgs['notifHammer']) &&
            count($notifPlayerArgs['notifHammer']) != 0
        ) {
            $this->generateNotifHammer(
                $player_id,
                $notifPlayerArgs['notifHammer']
            );
        }
    ...
    public function getRessourcesToTransform()
    {
        ...
        switch ($card_info['action']) {
            case 'fullThrow2Transform':
                $transform = [
                    'ressource' => ['fireshard', 'moonshard'],
                    'vp' => 2,
                ];
                break;
            case '4ThrowsTransform':
                $transform = ['ressource' => ['gold'], 'vp' => 1];
                break;
        }
    ...
    public function filterCartesian(
        $input,
        $maxValue,
        $vpDivide = 1,
        $hasTransformed,
    ...
    public function formatCartesian(
        $cartesian,
        $possibilities,
        $sideNum,
        $combine = false
    ...
    public function calculatePossibilities(
        ...
        if (
            $sideNum < 3 &&
            $this->gamestate->getCurrentMainState()->name == 'exploitRessource' &&
            !$misfortune
        ) {
        ...
        if ($side_definition['type'] == 'simple') {
            ...
            if (
                count(
                    array_intersect(
                        array_keys($side_definition['ressource']),
            ...
            if (
                isset($side_definition['ressource']['gold']) &&
                $side_definition['ressource']['gold'] != 0 &&
                ($hasHammer || in_array('gold', $toTransform['ressource']))
            ) {
                if ($cardAction != 'looseThrow') {
                    ...
                    $loyaltyReward = $this->getLoyaltyArg(
                        $player_id,
                        $sideNum,
                        'gold',
                        $misfortune
            ...
            foreach ($toTransform['ressource'] as $num => $res) {
                ...
                if (
                    isset($side_definition['ressource'][$res]) &&
                    $side_definition['ressource'][$res] != 0
                ) {
                    $value = $side_definition['ressource'][$res] * $triple;
                    $maxValue += $value;
                    $vp = $toTransform['vp'];
            ...
            $possibilities = $this->formatCartesian(
                $cartesian,
                $possibilities,
                $sideNum,
                true
            ...
            foreach ($side_definition['ressource'] as $element => $ressource) {
                ...
                if (
                    ...
                    if ($cardAction != 'looseThrow') {
                        if ($element == 'gold') {
                            $ressources[$ressourceToText[$element]][] =
                                $value +
                                $this->getLoyaltyArg(
                                    $player_id,
                                    $sideNum,
                            ...
                            $value += $this->getLoyaltyArg(
                                $player_id,
                                $sideNum,
                                'gold',
                                $misfortune
                            )['gold'];
                            $ressources['[VP]'][] = $this->getLoyaltyArg(
                                $player_id,
                                $sideNum,
                                'gold',
                                $misfortune
                            )['vp'];
                            $loyaltyBonus = $this->getLoyaltyArg(
                                $player_id,
                                $sideNum,
                                'gold',
                                $misfortune
     ...
     */
    public function canForgeSides($player_id, $remainingGold, $alreadyBought)
    {
        $sql =
            'SELECT MIN(card_id) FROM sides WHERE card_location BETWEEN 1 AND 10';

        if (count($alreadyBought) != 0) {
            $sql .=
                " AND card_type NOT IN ('" .
                implode("','", $alreadyBought) .
                "')";
        }
        ...
        if (
            $remainingGold + $tritonGold <
            $this->dice_sides[$side['type']]['cost']
        ) {
            return false;
        } else {
            return true;
     ...
     */
    public function listSidesWithExploits()
    {
        ...
        foreach ($garden as $i => $gard) {
            switch ($gard) {
                ...
                case 'boar':
                    if (
                        in_array('redBoar', $exploits) ||
                        in_array('greenBoar', $exploits) ||
                        in_array('yellowBoar', $exploits) ||
                        in_array('blueBoar', $exploits)
                    ) {
                        //$info[] = $gard;
                        $info[] = 'redBoar';
                        $info[] = 'yellowBoar';
     ...
     *
     */
    public function getNextState($ressourceChoice = null)
    {
    ...
    public function generateNotifLoyalty($notifPlayerArgs)
    {
    ...
    public function getTitanReward($player_id, $card_type)
    {
        $uniqueMessage = _(
            '${player_name} gets ${ressources} from the Titans for being the first to do accomplish this exploit'
        );
        $firstMessage = _(
            '${player_name} gets ${ressources} from the Titans for accomplishing this exploit for the first time'
        );
        $message = '';
        // based on position
                    ...
                    case 'loyaltyPassive1':
                        if ($resourceType == 'gold') {
                            ...
                            if (
                                ...
                                $notifLoyalty['gold'] =
                                    $notifLoyalty['gold'] + 1 - $scepter;
                                $notifLoyalty['scepter'] =
                                    $notifLoyalty['scepter'] + $scepter;
                            }
                        } elseif ($resourceType == 'hammer') {
                            $notifPlayerArgs[
                                'notifHammer'
                            ][] = $this->increaseGold($player_id, 1, 'hammer');
                            $notifLoyalty['hammer'] =
                                $notifLoyalty['hammer'] + 1;
                        }
                        break;
    ...
    public function getLoyaltyArg(
        $player_id,
        $sideNum,
        $resourceType,
        $misfortune = false
    ) {
        $argument = ['gold' => 0, 'vp' => 0];

        if ($this->getGameStateValue('rebellion') == 3) {
            if (
                ($this->isThrown($player_id, $sideNum) && !$misfortune) ||
                $misfortune
            ) {
                $position = $this->tokens->getTokenState(
                    'position_' . $player_id
    ...
    public function affectRessource(
        ...
        if (
            ...
            ($side_definition['type'] == 'dependent' && $allocateShield)
        ) {
            ...
            if (
                ...
                foreach (
                    $side_definition['ressource'] as $element => $ressource
                ) {
                    if ($triple) {
                    ...
                    if ($element == 'fireshard') {
                        ...
                        if (
                            ...
                            $multiple != -1 &&
                            ($ship &&
                                $this->isGoldLimitReached($player_id, $value))
                        ) {
                            if (!$doNotSetChoice) {
                                $this->dbSetSideChoice(
                                    $player_id,
                        ...
                        elseif (
                            ...
                            if (array_key_exists('gold', $notifPlayerArgs)) {
                                $notifPlayerArgs['gold'] =
                                    $notifPlayerArgs['gold'] + $remainingGold;
                            } else {
                                $notifPlayerArgs['gold'] =
                                    $remainingGold - $scepter;
                            }
                            ...
                            if (array_key_exists('hammer', $notifPlayerArgs)) {
                                $notifPlayerArgs['hammer'] =
                                    $notifPlayerArgs['hammer'] + $hammer;
                            } else {
                                $notifPlayerArgs['hammer'] = $hammer;
                            }

                            $notifPlayerArgs = $this->getLoyaltyReward(
                                $player_id,
                                $sideNum,
                                'gold',
                                $notifPlayerArgs
                        ...
                        if ($multiple != -1) {
                            ...
                            $this->tokens->incTokenState(
                                "movetitan_$player_id",
                                $value * -1
                            );
                        } else {
                            $this->tokens->setTokenState(
    ...
    public function affectTriple(
        ...
        foreach ($side_definition['ressource'] as $element => $ressource) {
            if (
                $this->tokens->getTokenState($element . '_' . $player_id) == 1
            ) {
                $value = $ressource * 2;
                if ($element == 'gold') {
                    if ($multiple != -1) {
            ...
            if (
                in_array($element, $toTransform['ressource']) &&
                $this->tokens->getTokenState('vp_' . $player_id == 1)
            ) {
                $vp = $toTransform['vp'];
                $value = $ressource * 2;
    ...
    public function sceptersToFill($player_id)
    {
        $max_value = 6;
        $scepters = [];
        ...
        $playerTokens = $this->tokens->getTokensOfTypeInLocation(
            'scepter%',
            $player_id,
            null,
            'token_state',
     ...
     */
    public function increaseGold($player_id, $quantity, $where = 'stock')
    {
    ...
    public function decreaseGold($player_id, $quantity, $action = null)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $res = ['gold' => 0, 'scepter' => 0];
    ...
    public function increaseMoonShard($player_id, $quantity)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $lost = 0;
    ...
    public function decreaseMoonShard($player_id, $quantity, $action = null)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $player_moon = $players_info[$player_id]['res_moon'];
    ...
    public function increaseAncientShard($player_id, $quantity)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $lost = 0;
    ...
    public function decreaseAncientShard($player_id, $quantity, $action = null)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $player_ancient = $players_info[$player_id]['res_ancient'];
        $new_ancient =
            $player_ancient < $quantity ? 0 : $player_ancient - $quantity;
    ...
    public function increaseFireShard($player_id, $quantity)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $lost = 0;
    ...
    public function decreaseFireShard($player_id, $quantity, $action = null)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
        $player_fire = $players_info[$player_id]['res_fire'];
        $new_fire = $player_fire < $quantity ? 0 : $player_fire - $quantity;
    ...
    public function increaseVP($player_id, $quantity)
    {
        return $this->dbIncreaseVP($player_id, $quantity);
    }

    public function decreaseVP($player_id, $quantity)
    {
     ...
     */
    public function increaseHammerPosition($player_id, $quantity)
    {
     ...
     * @return : boolean
     */
    public function hasUnusedShip($player_id = null)
    {
        $sql =
            "SELECT distinct 1 FROM player WHERE (side_choice_1 = 'ship' OR side_choice_2 = 'ship')";
    ...
    public function processedExploit()
    {
        $card_id = $this->getGameStateValue('exploitBought');
        if (
            $card_id != -1 &&
            ($this->gamestate->getCurrentMainState()->name == 'exploitRessource' ||
                $this->gamestate->getCurrentMainState()->name == 'exploitEffect')
        ) {
            $card = $this->exploits->getCard($card_id);
            $card_info = $this->exploit_types[$card['type']];
            //$exploit = true;
    ...
    public function hasResolutionConflict()
    {
        $sql =
        ...
        foreach ($dbres as $num => $result) {
            if ($result['card_type'] == 'mirror') {
                ...
                if (
                    count($ship_owned) != 0 &&
                    (count($ship_owned) == 1 &&
                        !in_array($result['player_id'], $ship_owned))
                ) {
                    return true;
                }
                ...
                if (
                    count($mirror_owned) != 0 &&
                    (count($mirror_owned) == 1 &&
                        !in_array($result['player_id'], $mirror_owned))
                ) {
                    return true;
                }
        ...
        $sql =
            "SELECT count(*) FROM sides WHERE (card_type = 'mirror' or card_type = 'ship') AND ";
        $sql .=
     ...
     * @return : boolean
     */
    public function isRessourceChoice(?ResourceChoice $action = null, $player_id = null)
    {
    ...
    public function hasUnresolvedSides($player_id = null)
    {
        $sql =
            "SELECT distinct 1 FROM player WHERE (side_choice_1 != '0' OR side_choice_2 != '0') ";
    ...
    public function isMazeChoice($player_id = null)
    {
    ...
    public function hasChest($player_id)
    {
        return $this->countExploitInLocation('chest', 'table-' . $player_id);
    }

    public function hasActiveHammer($player_id)
    {
        return $this->countExploitInLocation('hammer', 'table-' . $player_id);
    }

    public function hasBear($player_id)
    {
        return $this->countExploitInLocation('bear', 'pile2-' . $player_id);
    }

    public function resolveTwin($player_id, $toResolve)
    {
        if (
            $toResolve &&
            $this->getPlayersAdditionnalInfo()[$player_id]['twins'] != 0
        ) {
    ...
    public function hasHind($player_id)
    {
        return $this->countExploitInLocation('doe', 'pile3-' . $player_id);
    }

    public function hasElder($player_id)
    {
        return $this->countExploitInLocation('ancien', 'pile3-' . $player_id);
    }

    public function hasOwl($player_id)
    {
        return $this->countExploitInLocation('owl', 'pile3-' . $player_id);
    }

    public function hasCerberusToken($player_id = null)
    {
        $sql =
            "SELECT max(cerberus_token) FROM player, exploit WHERE card_location = concat('pile1-', player_id) AND exploit.card_type_arg = 0 ";
        $sql .=
            "AND card_type = 'cerberus' AND cerberus_token > 0 AND rolled = 1";
        if ($player_id != null) {
            $sql .= ' AND player_id = ' . $player_id;
        }
    ...
    public function getGold($player_id)
    {
    ...
    public function hasTritonToken($player_id)
    {
        $players = $this->getPlayersAdditionnalInfo();
    ...
    public function autoAffectTritonToken($player_id)
    {
    ...
    public function getActiveReinforcements($player_id)
    {
        $sql =
            "SELECT card_id AS id, card_type AS type
        FROM exploit
        WHERE card_type_arg = 0
        AND card_location = 'pile3-" .
            $player_id .
            "'";
    ...
    public function resetTwins($player_id = null, $used = false)
    {
        if ($used == false) {
            $sql =
    ...
    {
        $sql =
            "SELECT min(card_id) FROM exploit WHERE card_location = '" .
            $card_position .
            "'";
    ...
    public function getPlayersAdditionnalInfo()
    {
        $this->players_info = null;
    ...
    public function dbIncTwins($player_id, $add = true)
    {
        if ($add) {
            $sql =
                'UPDATE player set twins = twins + 1 WHERE player_id = ' .
                $player_id;
        } else {
            $sql =
                'UPDATE player set twins = twins - 1 WHERE player_id = ' .
    ...
    public function dbIncTriton($player_id, $add = true)
    {
        if ($add) {
            $sql =
                'UPDATE player set triton_token = triton_token + 1 WHERE player_id = ' .
                $player_id;
        } else {
            $sql =
                'UPDATE player set triton_token = triton_token - 1 WHERE player_id = ' .
    ...
    public function dbIncCerberus($player_id, $add = true)
    {
        if ($add) {
            $sql =
                'UPDATE player set cerberus_token = cerberus_token + 1 WHERE player_id = ' .
                $player_id;
        } else {
            $sql =
                'UPDATE player set cerberus_token = cerberus_token - 1 WHERE player_id = ' .
    ...
    public function dbUpdateExploitPlayed($card_id, $played)
    {
        if ($played) {
            $sql =
                'UPDATE exploit SET card_type_arg = 1 WHERE card_id = ' .
                $card_id;
        } else {
            $sql =
    ...
    public function dbUpdateTokenPlayed($player_id, $token, $played)
    {
        if ($played) {
            $sql =
                "UPDATE exploit SET card_type_arg = 1 WHERE card_type = '" .
                $token .
                "' AND card_location = 'pile1-" .
                $player_id .
                "'";
        } else {
            $sql =
                "UPDATE exploit SET card_type_arg = 0 WHERE card_type = '" .
                $token .
                "' AND card_location = 'pile1-" .
                $player_id .
                "'";
    ...
    public function dbUpdateRolled($player_id, $rolled)
    {
        if ($rolled) {
            $sql =
                'UPDATE player SET rolled = 1 where player_id = ' . $player_id;
        } else {
            $sql =
     ...
     */
    public function dbUpdateUnrolled()
    {
        $sql =
    ...
    public function setChoice($player_id, ?ResourceChoice $action, $boarAction = false, $debug = false)
    {
        ...
        elseif (
            ...
            $boar = $player['boar'];
            $twins = $player['twins'];
            $resolveTwin = $this->tokens->getTokenState(
                'resolveTwin_' . $player_id
            );
            $maze = false;
            $mazestock = $this->tokens->getTokenState(
                'mazestock_' . $player_id
            );
            $puzzle = $this->tokens->getTokenState('puzzle_' . $player_id);
            $celestial = $this->tokens->getTokenState('celestialRunning');
            $celestialChoice = $this->tokens->getTokenLocation(
                'celestial_choice'
            );
            $misfortune = 0;
            ...
            if ($resolveTwin == true) {
                ...
                (($side1 != '0' &&
                    $side1 != '-1' &&
                    $this->tokens->getTokenState("throw1_$player_id")) ||
                    ($side2 != '0' &&
                        $side2 != '-1' &&
    ...
    public function isGoldLimitReached($player_id, $res_gold)
    {
        $player_chests = $this->hasChest($player_id);
        $max_gold =
            self::MAX_GOLD +
            self::CHEST_GOLD * $player_chests +
            $this->tokens->countTokensInLocAndKey('scepter', $player_id) * 6;
        $player_gold = $this->getGold($player_id);
        //$max_gold = self::MAX_GOLD + ( self::CHEST_GOLD * $player_chests );
    ...
    public function dbSetForge($player_id, $value)
    {
        $sql =
            "UPDATE player SET forge = '" .
            $value .
            "' WHERE player_id = " .
            $player_id;
        return self::DbQuery($sql);
    ...
    public function dbSetSideChoice($player_id, $side_num, $value)
    {
        if ($side_num < 50) {
            $sql =
                'UPDATE player SET side_choice_' .
                $side_num .
                " = '" .
                $value .
                "' WHERE player_id = " .
    ...
    public function dbSetPosition($player_id, $value)
    {
        $sql =
            "UPDATE player SET position = '" .
            $value .
            "' WHERE player_id = " .
            $player_id;
        return self::DbQuery($sql);
    ...
    {
        $sql =
            'UPDATE player SET hammer_position = hammer_position + ' .
            $value .
            ' WHERE player_id = ' .
            $player_id;
        return self::DbQuery($sql);
    ...
    public function setAutoHammer($player_id, $enable)
    {
        if ($enable == 'enable') {
            $sql =
                'UPDATE player SET hammer_auto = 1  WHERE player_id = ' .
                $player_id;
        } else {
            $sql =
    ...
    {
        $sql =
            "SELECT COUNT(card_id) AS nb FROM exploit
        WHERE card_type = '" .
            $type .
            "'
        AND card_location = '" .
    ...
    public function checkValidSide($side, $player_id = null, $exclude_player_id = null)
    {
        $sql =
            "SELECT distinct card_id from sides, player where (card_id = player.throw_1 or card_id = player.throw_2) and card_type = '" .
            $side .
            "' ";
    ...
    public function checkValidVisibleSide(
        $side,
        $player_id = null,
        $exclude_player_id = null
    ) {
        $sql =
            "SELECT distinct card_id from sides where card_type = '" .
            $side .
            "' and card_location_arg = 0";
        ...
        if ($exclude_player_id != null) {
            $sql .=
                " and card_location not like 'dice%-p" .
                $exclude_player_id .
                "'";
        }
    ...
    public function takeRessource2(
        $player_id,
        $mode = 'blessing',
        $sideNum,
        $side,
        ...
        if (!$boar && !$twins && !$maze && !$celestial) {
            ...
            if (
                ...
                ($this->checkValidSide('triple', $player_id) &&
                    $mode != 'steal2' &&
                    !$this->canUseTwins($player_id))
            ) {
                $triple = 3;
        ...
        elseif ($maze) {
            ...
            $celestialSide = $this->tokens->getTokenLocation(
                'celestial_choice'
            );
        ...
        if (
            $this->dice_sides[$side]['type'] == 'choice' &&
            $mode != 'transform' &&
            (($gold != 0 &&
                ($moonshard != 0 ||
                    $fireshard != 0 ||
        ...
        if (
            $this->dice_sides[$side]['type'] == 'simple' &&
            !$mirror &&
            count($this->dice_sides[$side]['ressource']) != 1 &&
            ($moonshard != 0 ||
                $fireshard != 0 ||
                ($vp != 0 &&
                    $this->getLoyaltyArg($player_id, $sideNum, 'gold')['vp'] !=
        ...
        if (!$maze && !$celestial && !$misfortune) {
            ...
            if (
                ($this->doubleThrow($player_id) ||
                    $this->processedExploit() == 'steal2') &&
                !$boar &&
                !$twins &&
                !in_array($side, $this->shields) &&
                $otherSideType == '0'
            ) {
                $notifPlayerArgs = $this->checkShield(
                    $notifPlayerArgs,
                    $multiple,
                    false,
                    $exploit
        ...
        elseif (
            (($player_info['side_choice_1'] == 'ship' &&
                ($player_info['side_choice_2'] == '0' ||
                    $player_info['side_choice_2'] == 'triple')) ||
                ($player_info['side_choice_2'] == 'ship' &&
                    ($player_info['side_choice_1'] == '0' ||
                        $player_info['side_choice_1'] == 'triple')) ||
                ($player_info['side_choice_1'] == 'ship' &&
                    $player_info['side_choice_2'] == 'ship')) &&
            $player_info['boar'] == 0
        ) {
    ...
    public function affectGoldOnly(
        $player_id,
        $mode = 'blessing',
        $otherSideType,
        $sideNum
    ...
    public function generateNotifHammer($player_id, $notifHammer)
    {
    ...
    public function maxHammer($player_id, $value)
    {
    ...
    public function remainingHammer($player_id)
    {
        $hammer_position = $this->getPlayersAdditionnalInfo()[$player_id][
            'hammer_position'
        ];
     ...
     */
    public function buildRessourceNotif($args)
    {
        $hasScepter = false;
        $toAdd = '';
    ...
    public function canTakeSecondAction($player_id)
    {
        ...
        $toConvert = 0;
        $scepters = $this->tokens->getTokensOfTypeInLocation(
            'scepter_%',
            $player_id
        );
        ...
        // companion
        $companion = $this->tokens->getTokensOfTypeInLocation(
            'companion_%',
            $player_id
        );
    ...
    public function haveEnoughRessource($player_id, $fireshard, $moonshard)
    {
        $sql =
        ...
        if (
            $dbres['fire'] + $scepterFire + $dbres['ancient'] >= $fireshard &&
            $dbres['moon'] + $scepterMoon + $dbres['ancient'] >= $moonshard
        ) {
            return true;
        } else {
            return false;
    ...
    public function isIslandUsed($player_id, $island)
    {
        $sql =
            "SELECT player_id FROM `player` where position = '" .
            $island .
            "' and player_id != '" .
            $player_id .
            "'";
    ...
    public function canFillHammer($player_id)
    {
        if (
            $this->hasActiveHammer($player_id) * self::HAMMER_MAX_POSITION >
            $this->dbGetHammerPosition($player_id)
        ) {
            return true;
        } else {
            return false;
    ...
    public function blessing(
        ...
        if ($dice1 && $dice2 && $roll) {
            $result_sides = [
                $this->rollDice($player_id, 1),
                $this->rollDice($player_id, 2),
            ];
            ...
            $result_sides = [$this->rollDice($player_id, 1)];
            $this->tokens->setTokenState("throw1_$player_id", 1);
            $notifPlayerArgs['dice1'] = $result_sides[0]['type'];
            $otherSide = $player_info['side_choice_2'];
        } elseif (!$dice1 && $dice2 && $roll) {
            $result_sides = [$this->rollDice($player_id, 2)];
            $this->tokens->setTokenState("throw2_$player_id", 1);
            $otherSide = $player_info['side_choice_1'];
            $notifPlayerArgs['dice2'] = $result_sides[0]['type'];
        } elseif ($dice1 && $dice2 && !$roll) {
            $side = ['type' => $player_info['side_choice_1']];
            if ($side['type'] == '0') {
        ...
        if (($dice1 && !$dice2) || (!$dice1 && $dice2)) {
            ...
            if (
                $this->tokens->getTokenState(
                    "throw$otherDie" . '_' . $player_id
                ) &&
                $player_info["side_choice_$otherDie"] == '0' &&
                $result_sides[0]['type'] == 'triple'
            ) {
                $additionalSide = $this->sides->getCard(
                    $player_info["throw_$otherDie"]
                );
        ...
        if (
            $this->tokens->getTokenState("throw1_$player_id") == 1 &&
            $this->tokens->getTokenState("throw2_$player_id")
        ) {
            $doubleThrow = true;
        }
        ...
        if (
            !$additionalResource &&
            (($dice1 && !$dice2 && $notifPlayerArgs['dice1'] == 'triple') ||
                (!$dice1 && $dice2 && $notifPlayerArgs['dice2'] == 'triple') ||
                ($dice1 &&
                    $dice2 &&
        ...
        if (
            ...
            if ($result_sides[0]['type'] == 'mirror') {
                ...
                if (
                    substr($result_sides[1]['type'], -4) == 'Boar' &&
                    $this->getBoar($result_sides[1]['type']) != 0
                ) {
            ...
            if ($result_sides[1]['type'] == 'mirror') {
                ...
                if (
                    substr($result_sides[0]['type'], -4) == 'Boar' &&
                    $this->getBoar($result_sides[0]['type']) != 0
                ) {
        ...
        if ($dice1 && $dice2) {
            ...
            foreach ($result_sides as $id => $side) {
                ...
                if (
                    isset($side_definition['ressource']['loyalty']) &&
                    $side_definition['ressource']['loyalty'] != 0 &&
                    $side_definition['type'] != 'dependent'
                ) {
                    $hasTitan = true;
                }
                ...
                if (
                    isset($side_definition['ressource']['ancientshard']) &&
                    $side_definition['ressource']['ancientshard'] != 0 &&
                    $side_definition['type'] != 'dependent'
                ) {
                    $hasTitan = true;
                }
        ...
        foreach ($result_sides as $side_num => $side) {
            ...
            elseif (
                substr($side['type'], -4) == 'Boar' &&
                $this->getBoar($side['type']) != 0 &&
                ($roll || $cerberus)
            ) {
                //var_dump("toto");
                //throw new feException(debug_print_backtrace());
        ...
        if (!$this->canUseTwins($player_id) && $otherSide != '0') {
            ...
            // if a side is a boar, increment boar counter of the given player)
            elseif (
                substr($otherSide, -4) == 'Boar' &&
                $this->getBoar($otherSide) != 0 &&
                $this->getPlayersAdditionnalInfo()[$player_id]['twins'] >= 1
            ) {
        ...
        if (($ship || $maze) && $multiple != -1) {
            foreach ($result_sides as $id => $side) {
                ...
                if (
                    $this->isGoldLimitReached($player_id, $res_gold) &&
                    $res_gold > 0
                ) {
                    if ($dice1) {
                        $this->dbSetSideChoice(
        ...
        if ($additionalResource) {
            ...
            if ($notifPlayerArgs['choice'] == true) {
                $notifPlayerArgs = $this->affectTriple(
                    $player_id,
                    $notifPlayerArgs,
                    $otherDie,
                    $additionalSide['type'],
        ...
        if (
            $this->getGameStateValue('oracleReinforcement') == 1 &&
            ($notifPlayerArgs['loyalty'] != 0 ||
                $notifPlayerArgs['ancientshard'] != 0)
        ) {
     ...
     */
    public function mazeManagement($player_id, $new_position = null, $onlyOne = false)
    {
        ...
        for ($i = 0; $i < abs((int)$mazestock) + abs((int)$timeGolem); $i++) {
            ...
            if ($new_position == null) {
                if (
                    ($way == 'increase' && count($mazeSquare['path']) > 1) ||
                    ($way == 'decrease' &&
            ...
            if (
                $this->getGameStateValue('doubleCelestialRoll') == 0 &&
                $this->maze[$new_position]['reward'] == 'celestialRollx2'
            ) {
                if ($way == 'increase') {
                    $mazestock++;
            ...
            } elseif ($this->getGameStateValue('doubleCelestialRoll') == 1) {
                // && $this->maze[$position]['reward'] == 'celestialRollx2' ) {
                if ($way == 'increase') {
                    $this->tokens->incTokenState('mazestock_' . $player_id, -1);
                } else {
                    if ($timeGolem != 0) {
    ...
    public function triggerCelestialWhenMaze0($player_id)
    {
    ...
    public function mazeMove($player_id, $position, $next_position)
    {
        $increase = true;
        $timeGolem = $this->getGameStateValue('timeGolem');
        ...
        if (
            !(
                (in_array($next_position, $mazeSquare['path']) && $increase) ||
        ...
        if ($this->getGameStateValue('doubleCelestialRoll') == 1) {
            ...
            $msg = clienttranslate(
                '${player_name} moves in the Goddess\' Labyrinth to ${position}'
            );
        }
    ...
    public function mazeEffect($player_id)
    {
        $position = $this->tokens->getTokenState('position_' . $player_id);
        $mazeSquare = $this->maze[$position];
        $notifPlayerArgs = $this->initNotif($player_id);
        ...
        switch ($reward) {
            ...
            case 'forgeShip':
                ...
                if (
                    $this->gamestate->getCurrentMainState()->name == 'exploitEffect' ||
                    $this->gamestate->getCurrentMainState()->name == 'playerOustingChoice'
                ) {
                    $notifPlayerArgs['choice'] = true;
                } else {
                    $notifPlayerArgs['choice'] = false;
        ...
        if ($notifPlayerArgs['choice'] == true) {
            if (
                $this->getGameStateValue('mazeForge') == 0 &&
                !$forge &&
                !$firstFinish
            ) {
                $this->resourceChoiceHelper->dbSetChoice($player_id, ResourceChoice::RC_MAZE);
    ...
    public function mazeHasChoice($player_id, $mazePosition, $way)
    {
    ...
    public function doubleThrow($player_id)
    {
        if (
            $this->tokens->getTokenState("throw1_$player_id") &&
            $this->tokens->getTokenState("throw2_$player_id")
    ...
    public function isThrown($player_id, $side)
    {
    ...
    public function checkPuzzle($notifPlayerArgs)
    {
        ...
        $result_sides = [
            1 => $this->sides->getCard($player_info['throw_1']),
            2 => $this->sides->getCard($player_info['throw_2']),
        ];

        // mirror + twins management
        if ($this->tokens->getTokenState("mirror1_$player_id") != 0) {
            $result_sides[1] = $this->sides->getCard(
                $this->tokens->getTokenState("mirror1_$player_id")
            );
        }

        if ($this->tokens->getTokenState("mirror2_$player_id") != 0) {
            $result_sides[2] = $this->sides->getCard(
                $this->tokens->getTokenState("mirror2_$player_id")
            );
        }
        ...
        if (
            ...
            ($result_sides[2]['type'] == 'moonGolem' &&
                $result_sides[1]['type'] == 'sunGolem')
        ) {
    ...
    public function checkShield($notifPlayerArgs, $multiple, $ship, $exploit)
    {
        $player_id = $notifPlayerArgs['player_id'];
        $triple = $this->tokens->getTokenState("triple_$player_id");
        //$toTransform = array();
        ...
        if ($doubleThrow) {
            ...
            foreach ($result_sides as $side_num => $side) {
                $side_definition =
                    $this->dice_sides[$result_sides[$side_num]['type']];
                if ($side_definition['type'] == 'simple') {
            ...
            foreach ($result_sides as $side_num => $side) {
                ...
                if (
                    $this->tokens->getTokenState(
                        $side_definition['dependentRessource'] . "_$player_id"
                    )
                ) {
                    foreach ($side_definition['gainA'] as $res => $value) {
                        switch ($res) {
                            case 'vp':
                                ...
                                $notifPlayerArgs['vp'] =
                                    $notifPlayerArgs['vp'] + $value * $multiple;
                            ...
                            case 'loyalty':
                                ...
                                $notifPlayerArgs['loyalty'] =
                                    $notifPlayerArgs['loyalty'] +
                                    $value * $multiple;
                                break;
                            case 'ancientshard':
                                if ($multiple != -1) {
                                    $this->increaseAncientShard(
                                        $player_id,
                                        $value
                                    );
                                    // value * -1 because ancient shard must move back
            ...
            if (
                $this->getPlayersAdditionnalInfo()[$player_id]['boar'] == 0 &&
                $multiple != -1
            ) {
                $this->setChoice($player_id, null);
    ...
    public function scoreForgedSides($player_id, $exploit = true)
    {
        $discardedSides = count(
            $this->sides->getCardsInLocation('discard', $player_id)
        );
    ...
    public function titanMove($player_id)
    {
        if ($this->getGameStateValue('rebellion') == 3) {
            $titanStock = $this->tokens->getTokenState(
                'movetitan_' . $player_id
            );
            $player_info = $this->getPlayersAdditionnalInfo()[$player_id];
            $initialPosition = $this->tokens->getTokenState(
                'position_' . $player_id
            );
            ...
            for ($i = 0; $i < abs((int)$titanStock); $i++) {
                ...
                if (
                    ($position == 1 && $way == 'decrease') ||
                    ($position == 31 && $way == 'increase')
                ) {
                ...
                if ($way == 'decrease') {
                    $this->tokens->setTokenState(
                        'position_' . $player_id,
                        $this->titan_board[$position]['toDark']
                    );
                    $this->tokens->incTokenState('movetitan_' . $player_id, 1);
                } elseif ($way == 'increase') {
                    $this->tokens->setTokenState(
                        'position_' . $player_id,
                        $this->titan_board[$position]['toLight']
                    );
                    $this->tokens->incTokenState('movetitan_' . $player_id, -1);
                }
                $lastPosition = $this->tokens->getTokenState(
                    'position_' . $player_id
                );
            }
    ...
    public function actDraft($exploit)
    {
        ...
        // get the slot
        $slot =
            $this->exploit_slot[$this->getGameStateValue('slotBeingDrafted')];
    ...
    public function actSideChoice($side1, $side2, $side98)
    {
        ...
        $doubleMirror = $this->checkValidVisibleSide(
            'mirror',
            null,
            $player_id
        );
            ...
            if ($side1 != null) {
                ...
                if (
                    ...
                    if (
                        ...
                        $this->gamestate->setPlayersMultiactive([$boarId], '');
                    }
                }
                // Misfortune
        ...
        if ($chooseSides && !$celestial) {
            ...
            $old_side = $this->sides->getCardsInLocation(
                'dice' . $celestialDieNum . '-p' . $player_id,
                0
            );
            $old_side = reset($old_side);
            ...
            $this->tokens->setTokenState(
                'throw' . $celestialDieNum . "_$player_id",
                1
            );
        }
        ...
        if (
            ($checkMirror_1 || ($steal2 && !$celestial) || $oustAll) &&
            !$chooseSides
        ) {
            if (
                !$oustAll &&
        ...
        if (strpos($side1, 'Golem') && $multiple != -1) {
            ...
            $maze = true;
        } elseif (strpos($side1, 'Golem') && $multiple == -1) {
            $this->dbSetSideChoice($player_id, 1, '0');
        }
        ...
        if (strpos($side2, 'Golem') && $multiple != -1) {
            ...
            $maze = true;
        } elseif (strpos($side2, 'Golem') && $multiple == -1) {
            $this->dbSetSideChoice($player_id, 2, '0');
        }
        ...
        if (
            (isset($notifPlayerArgs['dice1']) &&
                isset($notifPlayerArgs['dice2']) &&
                $chooseSides) ||
            $steal2
        ) {
            $notifPlayerArgs = $this->checkPuzzle($notifPlayerArgs);
        ...
        if (
            $this->gamestate->getCurrentMainState()->name == 'doeRessourceChoice' &&
            ($player_info['side_choice_1'] != '0' ||
                $player_info['side_choice_2'] != '0')
        ) {
            $this->setGameStateValue('celestialRunning', 0);
        ...
        elseif (
            ...
            // we disable the user
            switch ($this->gamestate->getCurrentMainState()->name) {
                case 'ressourceChoice':
                    $endMultiState = 'blessing';
                    break;
                case 'doeRessourceChoice':
    ...
    public function oustingMazeManagement($player_id)
    {
        // trigger maze management for ousting after a choice
        $this->setGameStateValue('celestialRunning', 0);
        $canContinue = $this->mazeManagement($player_id);
    ...
    public function actForgeShipPass($sideNum)
    {
        ...
        if (
            $player_info['side_choice_' . $otherSide] == '0' ||
            $player_info['side_choice_' . $otherSide] == 'triple'
        ) {
            $this->dbSetSideChoice($player_id, $otherSide, 0);
            $this->tokens->setTokenState("triple_$player_id", 0);
        }
        ...
        if (
            $player_info['side_choice_' . $otherSide] != '0' &&
            $player_info['side_choice_' . $otherSide] != 'triple' &&
            $player_info['side_choice_' . $otherSide] != 'ship' &&
            !$this->canUseTwins($player_id)
        ) {
    ...
    public function reallocateScepter($player_id, $value)
    {
        $scepters = $this->tokens->getTokensOfTypeInLocation(
            'scepter_%',
            $player_id,
    ...
    public function actUseTritonToken($ressources)
    {
        ...
        // Control that only one ressource has been choosen
        if (
            ($gold != 0 && ($moonshard != 0 || $fireshard != 0 || $vp != 0)) ||
        ...
        if (
            $this->gamestate->getCurrentMainState()->name == 'secondAction' &&
            !$this->hasTritonToken($player_id) &&
            !$this->canTakeSecondAction($player_id)
        ) {
            $this->gamestate->nextState('endPlayerTurn');
        } elseif ($this->gamestate->getCurrentMainState()->name == 'reinforcement') {
    ...
    public function actUseCompanion($card_id)
    {
        ...
        if (
            $this->gamestate->getCurrentMainState()->name == 'secondAction' &&
            !$this->hasCompanionToken($player_id) &&
            !$this->canTakeSecondAction($player_id)
        ) {
            $this->gamestate->nextState('endPlayerTurn');
        } else {
    ...
    public function actUseCerberusToken($use)
    {
        ...
        if (!$use) {
            ...
            if ($this->gamestate->getCurrentMainState()->name == 'doeRessourceChoice') {
            ...
            if ($dice_number == 1) {
                ...
                $displaySides =
                    ...
                    ',' .
                    $this->sides->getCard($player_info['throw_2'])['type'];
            }
            ...
            switch ($stateName) {
                ...
                case 'exploitRessource':
                    ...
                    switch ($card_info['action']) {
                        ...
                        case '4Throws':
                        case '4ThrowsTransform':
                            $die_number = $this->getGameStateValue(
                                'enigmaDieNumber'
                            );
                            if ($die_number == 1) {
                                $choice = $this->blessing(
            ...
            switch ($action) {
                case 'ressource':
                    if ($side == 'ship') {
                        ...
                        $player_info = $this->getPlayersAdditionnalInfo()[
                            $player_id
                        ];
                        $side = $player_info['side_choice_' . $die];
    ...
    public function actActionMisfortune($action, $die, $goldSources = null)
    {
        ...
        if ($die != 0) {
            ...
            $otherSide = $this->tokens->getTokenLocation(
                'misfortune_' . $otherDie
            );
            switch ($action) {
                case 'ressource':
        ...
        if ($card_id != 0) {
            ...
            switch ($card_type) {
                ...
                case 'merchant':
                    ...
                    $nbMerchant = $this->countExploitInLocation(
                        'merchant',
                        "pile3-$player_id"
                    );
                    ...
                    if ($merchant_parameters['nbUpgrade'] > 0) {
                        $this->checkUpgrade(
                            $merchant_parameters['nbUpgrade'],
                            $merchant_parameters['old_side'],
                    ...
                    foreach (
                        $this->exploits->getCardsOfType('merchant') as $card
                    ) {
    ...
    public function oracleMove($player_id)
    {
        $players_info = $this->getPlayersAdditionnalInfo();
    ...
    public function actBuyExploit($card_id, $ressources = null)
    {
        ...
        if (!$hasPreviousExploit) {
            ...
            $ousted_player = $this->isIslandUsed(
                $player_id,
                $card_info['island']
            );
        ...
        if (!$hasPreviousExploit) {
            $msg = clienttranslate(
                '${player_name} has bought ${card_name_trans} with ${ressources}'
            );
        } else {
            $msg = clienttranslate(
                '${player_name} has bought ${card_name_trans} for free'
            );
        }
    ...
    public function actBuyForge($toForge, $toReplace, $mode = 'classic')
    {
        ...
        if (
            ...
            $player_info['side_choice_2'] == 'ship' ||
            $this->getGameStateValue('mazeForge') == 1 ||
            $this->tokens->getTokenLocation('celestial_choice') == 'ship'
        ) {
            $ship = true;
        ...
        if ($type_arg != 0) {
            $sql =
                'UPDATE sides SET card_type_arg = ' .
                $type_arg .
                ' WHERE card_id = ' .
                $toForge;
            self::DbQuery($sql);
        ...
        if ($cost == 0) {
            $desc = clienttranslate(
                '${player_name} has forged ${side_type} on dice ${dice_number}, ${old_side_type} is discarded'
            );
        }
        ...
        if ($ship) {
            ...
            if ($ressourceChoice != ResourceChoice::RC_FORGESHIP->value) {
                ...
                if (
                    $this->gamestate->getCurrentMainState()->name == 'oustedForgeShip' &&
                    $this->tokens->getTokenState("mazestock_$player_id") != 0
                ) {
                    if ($this->getGameStateValue('celestialRunning')) {
                        $this->setGameStateValue('celestialRunning', 0);
    ...
    public function actCancelCelestial()
    {
        ...
        if ($card_id != -1) {
            ...
            if (
                $card_info['action'] == 'throwCelestialDie' &&
                $this->getGameStateValue('exploitRemainingThrows') > 0
            ) {
                $this->incGameStateValue('exploitRemainingThrows', -1);
            }
        }
    ...
    public function actExploitRessource($sideNum, $side, $ressources)
    {
        ...
        } elseif ($this->getGameStateValue('celestialRunning')) {
            $mode = 'dice1';
            if (
                $card_info['action'] == 'fullThrow2Transform' ||
                $card_info['action'] == '4ThrowsTransform'
            ) {
                $mode = 'transform';
            }
            $disable = $this->takeRessource2(
            ...
            switch ($card_info['action']) {
                ...
                case 'throwAllChooseResources':
                    ...
                    foreach ($players_info as $it_player_id => $player_info) {
                        for ($i = 1; $i < 3; $i++) {
                            $side = $this->sides->getCard(
                                $player_info['throw_' . $i]
                            );
                            $side_info = $this->dice_sides[$side['type']];

                            foreach (
                                $side_info['ressource'] as $res => $value
                            ) {
    ...
    public function actSecondAction($to_play, $resources)
    {
        ...
        if ($to_play) {
            ...
            // check enough FS
            if (
                ($this->tokens->countTokensInLocAndKey(
                    'scepter_',
                    $player_id
                ) != 0 ||
                    $this->tokens->countTokensInLocAndKey(
    ...
    public function misfortuneAllocation()
    {
        ...
        if ($this->isMisfortune($side1['type']) != 0) {
            ...
            if (
                substr($side1['type'], -4) == 'Boar' &&
                $this->getBoar($side1['type']) != 0
            ) {
            ...
            if (
                (isset($side_def['gold']) && $side_def['gold'] != 0) ||
                (isset($side_def['vp']) && $side_def['vp'] != 0)
            ) {
        ...
        if ($this->isMisfortune($side2['type']) != 0) {
            ...
            if (
                substr($side2['type'], -4) == 'Boar' &&
                $this->getBoar($side2['type']) != 0
            ) {
    ...
    */
    ...
    public function argsReinforcement()
    {
        ...
        foreach ($reinforcements as $key => $reinforcement) {
            switch ($reinforcement['type']) {
                ...
                case 'companion':
                    if (
                        $this->tokens->getTokenState('companion_' . $key) >= 5
                    ) {
                        $reinforcements[$key]['unusable'] = true;
                    }
                    break;
    ...
    public function argsSecondAction()
    {
        $companions = [];
        foreach ($this->exploits->getCardsOfType('companion') as $card) {
            if ($card['location'] == 'pile3-' . $this->getActivePlayerId()) {
    ...
    public function argsMisfortune()
    {
    ...
    public function argsRessourceChoice()
    {
        $msgLoyalty = clienttranslate('(including Loyalty reward)');
        ...
        foreach ($this->gamestate->getActivePlayerList() as $player_id) {
            ...
            if ($player_info['ressource_choice'] == ResourceChoice::RC_SIDE_CHOICE->value) {
                ...
                if (
                    $this->getGameStateValue('firstFinish') == $player_id ||
                    $this->getGameStateValue('goddessChoice') == 1
                ) {
                ...
                if ($side1) {
                    $tmp = $this->sides->getCardsInLocation(
                        'dice1-p' . $player_id,
                        0
                    );
                    $tmp = reset($tmp);
                ...
                if (
                    $mirror != 0 &&
                    $this->checkValidSide('mirror', null, $player_id)
                ) {
                ...
                if (
                    $this->tokens->getTokenState('resolveTwin_' . $player_id) &&
                    $players_info[$player_id]['twins'] != 0
                ) {
                    $sides[$player_id]['sides'][0] = [
                        'id' => '',
                        'type' => 'twins',
                ...
                $twinChoice = $this->tokens->getTokenState(
                    'twinChoice_' . $player_id
                );
                if ($twinChoice != 0) {
                    $sides[$player_id]['sides'][0] = [
                ...
                elseif (
                    ...
                    $sides[$player_id]['sides'][0] = [
                        'id' => '',
                        'type' => $players_info[$player_id]['side_choice_1'],
                        'num' => 1,
                    ];
                    $sideNum = 1;
                } elseif (
                    ...
                    $sides[$player_id]['sides'][0] = [
                        'id' => '',
                        'type' => $players_info[$player_id]['side_choice_2'],
                        'num' => 2,
                    ];
                    $sideNum = 2;
                }
    ...
    public function argsForgeShip()
    {
        ...
        foreach ($this->gamestate->getActivePlayerList() as $player_id) {
            ...
            if ($this->getGameStateValue('mazeForge')) {
                ...
                $retour[$player_id]['sides'][0] = [
                    'id' => '',
                    'type' => $player_info['side_choice_1'],
                    'num' => 1,
                ];
            } elseif ($player_info['side_choice_2'] == 'ship') {
                $retour[$player_id]['sides'][0] = [
                    'id' => '',
                    'type' => $player_info['side_choice_2'],
                    'num' => 2,
                ];
            }
    ...
    public function argExploitEffect()
    {
        $card_id = $this->getGameStateValue('exploitBought');
        $pow = '';
        $pow_you = '';
        ...
        if ($card_id != -1) {
            ...
            if (isset($this->powers_you[$card_info['action']])) {
                $pow_you = clienttranslate(
                    $this->powers_you[$card_info['action']]
                );
            }
    ...
    public function argExploitRessource()
    {
        $card_id = $this->getGameStateValue('exploitBought');
        $players_info = $this->getPlayersAdditionnalInfo();
        $card = $this->exploits->getCard($card_id);
            ...
            if ($player_info['ressource_choice'] == ResourceChoice::RC_SIDE_CHOICE->value) {
                ...
                if ($side1) {
                    ...
                    if ($tmp['type'] == 'mirror') {
                        ...
                        $card_info['action'] == 'oustAll' &&
                        $players_info[$player_id]['side_choice_1'] == -1
                    ) {
                        $mirror++;
                    }
                }
                ...
                if ($side2) {
                    ...
                    if ($tmp['type'] == 'mirror') {
                        ...
                        $card_info['action'] == 'oustAll' &&
                        $players_info[$player_id]['side_choice_2'] == -1
                    ) {
                        $mirror++;
                    }
                }
                ...
                switch ($card_info['action']) {
                    ...
                    case 'throwAllChooseResources':
                        ...
                        // take all side up
                        foreach (
                            $players_info as $it_player_id => $player_info
                        ) {
                            for ($i = 1; $i < 3; $i++) {
                                $side = $this->sides->getCard(
                                    $player_info['throw_' . $i]
                        ...
                        if ($resource['fireshard'] != 0) {
                            $possibilities[$pos++] = [
                                '[G]' => 0,
                                '[H]' => 0,
                                '[FS]' => $resource['fireshard'],
                                '[MS]' => 0,
                                '[VP]' => 0,
                                '[AS]' => 0,
                                '[M]' => 0,
                                '[L]' => 0,
                                'text' => $resource['fireshard'] . ' [FS]',
                                'num' => 5,
                            ];
                        }
                        if ($resource['moonshard'] != 0) {
                            $possibilities[$pos++] = [
                                ...
                                '[H]' => 0,
                                '[FS]' => 0,
                                '[MS]' => $resource['moonshard'],
                                '[VP]' => 0,
                                '[AS]' => 0,
                                '[M]' => 0,
                                '[L]' => 0,
                                'text' => $resource['moonshard'] . ' [MS]',
                                'num' => 5,
                            ];
                        }
                        if ($resource['vp'] != 0) {
                        ...
                        if ($resource['ancientshard'] != 0) {
                            $possibilities[$pos++] = [
                                ...
                                '[L]' => 0,
                                '[M]' => 0,
                                '[AS]' => $resource['ancientshard'],
                                'text' => $resource['ancientshard'] . ' [AS]',
                                'num' => 5,
                            ];
                        }

                        $possibilities = $this->possibilityAddText(
                            $possibilities
                        );
    ...
    */
    ...
    public function stBeginPlayerTurn()
    {
        ...
        if ($magicSeagullActive == 1 && $magicSeagull == 0) {
            ...
            foreach ($table as $player => $previous_player) {
                $sql =
                    "UPDATE sides set card_location = 'dice1-p" .
                    $player .
    ...
    public function stBlessing()
    {
        ...
        if (
            $this->isRessourceChoice() ||
            $this->hasMazeStock() ||
            $this->hasUnresolvedSides() ||
            $this->misfortuneState() != 0
        ) {
    ...
    public function stRessourceChoice()
    {
        $stateName = $this->gamestate->getCurrentMainState()->name;
        $nextState = '';
        $endMultiState = '';
    ...
    public function stMisfortuneChoice()
    {
    ...
    public function stRessourceChoiceAdvanced($activeplayers = null, $continue = false)
    {
        // there is a ressource Choice on one of the players
        if (
            $this->isRessourceChoice() &&
    ...
    public function stReinforcement()
    {
        ...
        foreach (
            ...
            if (
                $reinforcement['type_arg'] == '0' &&
                ((($reinforcement['type'] == 'ancient' ||
                    $reinforcement['type'] == 'light') &&
                    ($this->getGold($player_id) >= 3 ||
                        $this->hasTritonToken($player_id))) ||
                    $reinforcement['type'] != 'ancient')
            ) {
    ...
    public function stOusting()
    {
        ...
        if ($this->misfortuneState() != 0) {
            ...
            $this->stRessourceChoiceAdvanced(
                $this->gamestate->getActivePlayerList(),
                true
            );
        } else {
            ...
            if (
                ...
                for ($i = 1; $i <= 2; $i++) {
                    $side_definition =
                        $this->dice_sides[$player_info['side_choice_' . $i]];
                    if (
                        isset($side_definition['ressource']['gold']) &&
                        $side_definition['ressource']['gold'] != 0
                    ) {
    ...
    public function stEffectExploit()
    {
        ...
        elseif ($this->hasUnresolvedSides()) {
            ...
            if (
                $card_info['actionType'] == 'immediate' ||
                $card_info['actionType'] == 'recurrent'
            ) {
                switch ($card_info['action']) {
                    case 'initHammer':
                    ...
                    case 'steal2':
                        ...
                        foreach (
                            $this->loadPlayersBasicInfos() as $all_player_id => $players
                        ) {
                    ...
                    case 'stealLessGold':
                        ...
                        foreach ($players as $aff_player_id => $player) {
                            ...
                            $av_vp =
                                $players_info[$aff_player_id]['player_score'];
                            $reduce = 0;
                    ...
                    case 'stealFireMoon':
                        ...
                        $gain['moonshard'] = 0;
                        foreach (
                            $players_info as $aff_player_id => $player_info
                        ) {
                            if ($player_id != $aff_player_id) {
                                $lost = [];
                    ...
                    case 'countVP':
                        ...
                        for ($num = 1; $num <= 2; $num++) {
                            foreach (
                                $this->sides->getCardsInLocation(
                                    'dice' . $num . '-p' . $player_id
                                ) as $id => $side
                            ) {
                                $side_info = $this->dice_sides[$side['type']];

                                if (
                                    isset($side_info['ressource']['vp']) &&
                                    $side_info['ressource']['vp'] != 0
                                ) {
                                    $nbSide++;
                                } elseif (
                                    isset($side_info['gainA']['vp']) &&
                                    $side_info['gainA']['vp'] != 0
                                ) {
                                    $nbSide++;
                                }
                            }
                    ...
                    case 'forgeEverywhere':
                        //trident
                        // loose all gold
                        $gold = $this->getPlayersAdditionnalInfo()[$player_id][
                            'res_gold'
                        ];
                    ...
                    case 'oustAll':
                        ...
                        $old_ousted = $this->getGameStateValue(
                            'oustedPlayerId'
                        );
                        $players_info = $this->getPlayersAdditionnalInfo();
                        ...
                        if (
                            $old_ousted == '-1' ||
                            $tableOrder[$old_ousted] != $player_id
                        ) {
                            ...
                            if ($players['position'] != 'begin') {
                                ...
                                if (
                                    $this->hasBear($all_player_id) != 0 &&
                                    $all_player_id != $player_id
                                ) {
                    ...
                    case 'freeExploit':
                        ...
                        if (
                            $this->exploits->countCardInLocation('F1') == 0 &&
                            $this->exploits->countCardInLocation('F2') == 0 &&
                            $this->exploits->countCardInLocation('M1') == 0 &&
                    ...
                    case 'memoryTokens':
                        ...
                        if ($remainingThrows == 3) {
                            $tokensToInit[] = [
                                'key' => $card['type'] . '_1_' . $player_id,
                                'nbr' => 1,
                                'state' => 0,
                            ];
                            $tokensToInit[] = [
                                'key' => $card['type'] . '_2_' . $player_id,
                                'nbr' => 1,
                                'state' => 0,
                            ];
                            $this->tokens->createTokens($tokensToInit, 'none');
        ...
        if (
            ($remainingThrows <= 0 &&
                ...
                !$this->hasMazeStock($player_id) &&
                !$forge &&
                !$this->hasUnresolvedSides($player_id)) ||
            // #35045
            ...
            if ($card_id != -1) {
                ...
                $card_info = $this->exploit_types[$card['type']];
                $island = $card_info['island'];
                $memoryTokens = $this->tokens->getTokensOfTypeInLocation(
                    '%Memory%' . $player_id,
                    $island
                );
    ...
    public function misfortuneState()
    {
    ...
    public function stEndPlayerTurn()
    {
        ...
        foreach (
            $this->exploits->getCardsInLocation('pile3-' . $player_id) as $card_id => $reinforcement
        ) {
            if (
                $reinforcement['type'] != 'companion' ||
                $this->tokens->getTokenState('companion_' . $card_id) < 5
            ) {
        ...
        if (!$eternalFire) {
            ...
            if ($currentPlayerNum == $this->getGameStateValue('nbPlayers')) {
                if ($turnCount < $nbTurns) {
                    ...
                    $this->setGameStateValue(
                        'currentPlayerNum',
                        $this->getGameStateValue('nbPlayers') + 1
                    );
    ...
    public function stEndScoring()
    {
        ...
        foreach ($players as $player_id => $player) {
            ...
            foreach ($locations as $location) {
                foreach (
                    $this->exploits->getCardsInLocation($location) as $card
                ) {
    ...
    public function zombieTurn($state, $active_player)
    {
        ...
        if ($state['type'] == 'activeplayer') {
            ...
            if ($statename == 'exploitForgeBoar') {
                ...
                $sides = $this->sides->getCardsInLocation(
                    'dice1-p' . $active_player
                );
    ...
    */
    ...
    public function debugNextState($state)
    {
