...
{
    public $table;
    public $autoreshuffle = false; // If true, a new deck is automatically formed with a reshuffled discard as soon at is needed
    public $autoreshuffle_trigger = null; // Callback to a method called when an autoreshuffle occurs
    ...
    public function createTokens($tokens, $location_global, $token_state_global = null)
    {
        if ($location_global) {
    ...
    public function getExtremePosition($getMax, $location, $token_key = null)
    {
    ...
    public function pickTokensForLocation($nbr, $from_location, $to_location, $state = 0, $no_deck_reform = false)
    {
     ...
     */
    public function getTokenOnTop($location)
    {
     ...
     */
    public function getTokensOnTop($nbr, $location)
    {
        self::checkLocation($location);
    ...
    public function reformDeckFromDiscard($from_location)
    {
    ...
    // Set token state
    public function setTokenState($token_key, $state)
    {
    ...
    public function incTokenState($token_key, $state)
    {
        self::checkState($state);
        self::checkKey($token_key);
        $sql = "UPDATE " . $this->table;
    ...
    public function moveToken($token_key, $location, $state = 0)
    {
        self::checkLocation($location);
        self::checkState($state);
        self::checkKey($token_key);
    ...
    public function insertToken($token_key, $location, $state = 0)
    {
        self::checkLocation($location);
        self::checkState($state);
        $sql = "UPDATE " . $this->table;
    ...
    public function moveAllTokensInLocation($from_location, $to_location, $from_state = null, $to_state = 0)
    {
     ...
     */
    public function moveAllTokensInLocationKeepOrder($from_location, $to_location)
    {
        self::checkLocation($from_location);
        self::checkLocation($to_location);
        $sql = "UPDATE " . $this->table;
     ...
     */
    public function getTokensInLocation($location, $state = null, $order_by = null)
    {
        return $this->getTokensOfTypeInLocation(null, $location, $state, $order_by);
    }
    public function getTokenOnLocation($location)
    {
     ...
     * @return array mixed
     */
    public function getTokensOfTypeInLocation($type, $location = null, $state = null, $order_by = null, $asc = true)
    {
        $sql = $this->getSelectQuery();
        $sql .= " WHERE true ";
        if ($type !== null) {
    ...
    public function getTokenState($token_id)
    {
    ...
    public function getTokenLocation($token_id)
    {
        $res = $this->getTokenInfo($token_id);
        if ($res == null) {
            return null;
     ...
     */
    public function getTokensInfo($tokens_array)
    {
    ...
    public function countTokensInLocation($location, $state = null)
    {
        self::checkLocation($location, true);
        self::checkState($state, true);
        $like = "LIKE";
    ...
    public function countTokensInLocAndKey($key, $location, $state = null)
    {
        self::checkLocation($location, true);
        self::checkState($state, true);
        $like = "LIKE";
        ...
        $sql = "SELECT COUNT( token_key ) cnt FROM " . $this->table;
        $sql .= " WHERE token_location $like '$location' and token_key LIKE '$key%'";
        if ($state !== null) {
            $sql .= "AND token_state='$state' ";
        }
        $dbres = self::DbQuery($sql);
    ...
    public function countTokensInLocations()
    {
        $result = array();
    ...
    public function countTokensByState($location)
    {
        self::checkLocation($location);
        $result = array();
    ...
    public function setCustomFields($fields_array)
    {
        $this->checkTokenKeyArray($fields_array);
        $this->custom_fields = $fields_array;
    }
    public function initGlobalIndex($key, $value = 1)
    {
    ...
    public function syncGlobalIndex($key)
    {
        $this->checkKey($key);
