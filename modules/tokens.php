...
class Tokens extends APP_GameClass {
    ...
    function incTokenState($token_key, $state) {
        self::checkState($state);
        self::checkKey($token_key);
        $sql = "UPDATE " . $this->table;
     ...
     * @return array mixed
     */
    function getTokensOfTypeInLocation($type, $location = null, $state = null, $order_by = null, $asc = true) {
        $sql = $this->getSelectQuery();
        $sql .= " WHERE true ";
        if ($type !== null) {
    ...
    function countTokensInLocAndKey($key, $location, $state = null) {
        self::checkLocation($location, true);
        self::checkState($state, true);
        $like = "LIKE";
        if (strpos($location, "%") === false) {
