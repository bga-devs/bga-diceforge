...
class action_diceforge extends APP_GameAction
{
    ...
    public function actPegasusIsland() {
        self::setAjaxMode();
		$island = self::getArg( "island", AT_int, true );

		$this->game->actPegasusIsland($island);
		self::ajaxResponse( );
    }

    public function actPegasusMinor()
	{
		self::setAjaxMode();
		$dice_num = self::getArg( "dice_num", AT_int, true );

		$this->game->actPegasusMinor($dice_num);
		self::ajaxResponse( );
	}
